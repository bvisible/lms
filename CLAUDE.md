...
⚠️ **Ne jamais lancer `yarn build` ou `bench build --app lms` localement sur un serveur Neoffice** (4 GB RAM → OOM-kill garanti). Le build se fait UNIQUEMENT sur GitHub Actions (ubuntu-latest, 16 GB RAM).
...
3. Sur les instances clients, le pipeline d'update fait `git pull` (ramène ton commit + le commit du bot). Quand `bench build --app lms` tourne, il appelle `yarn build` à la racine — **le `package.json` voit les artefacts déjà présents et skip vite** (gate). Plus d'OOM-kill.

### Paths spécifiques

- **Source frontend** : `frontend/`
- **Artefacts vite (commités)** : `lms/public/frontend/`
- **SPA HTML(s) (commités)** : `lms/www/lms.html` + `lms/www/_lms.html`
...
### Edge cases spécifiques à lms

- **socket.js**: import statique de `common_site_config.json` non géré côté source. Le workflow GH Action crée un stub `{"socketio_port": 9000}` à `<work>/sites/common_site_config.json` avant le build.
